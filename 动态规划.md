...
```  
### 2.最长公共子序列二    
这道题需要输出公共子序列，首先用动态规划来找到子序列的最长长度，然后从最后一格开始搜索长度递减的情况如果遇到所对应字符不相等，就用前面动规的逆向思维，找从dp[i - 1][j] 和dp[i][j - 1]中找，必须往dp值不变的方向移动，不然可能会移动到另外一个长度相等的却后面接不上的子序列。也就是如果遇到字符相等，就往斜对角走（i - 1， j - 1）,如果不相等，则往dp值不变的方向移动。
```cpp
