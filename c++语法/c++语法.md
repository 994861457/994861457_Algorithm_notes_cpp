...
## 1.8 内存泄漏  
# 2.static     
所有静态变量都在编译的时候分配内存并初始化，在程序运行的时候已经初始化完毕了（c中）。而c++中静态成员对象初始化在用到的时候，因为要调用构造函数。
作用：  
1.隐藏，将全局变量变为文件外不可见  
2.初始化为0，静态变量会在全局静态区初始化为默认值0等
## 2.1  在类外    
全局静态变量作用域为本文件内  
局部静态变量在函数结束运行以后在函数外无法访问但是那块内存会保留下来，下次调用该函数的时候会继续使用。
https://blog.csdn.net/weixin_38739598/article/details/109457944
## 2.2 类的静态成员变量
类的静态成员变量的生命周期是程序的运行周期，隶属于类，被对象共享，在全局区分配空间。在类外初始化，因为其存在先于对象   
## 2.3 类的静态成员函数   
隶属于类，所以没有this指针，所以不能访问非静态的成员变量。  
也不能被设置成虚函数，因为没有任何意义，其隶属于类而不是对象，也没有this指针，虚函数的实现需要依靠this指针。
# 3.面向对象   
...
# 3.c++11新特性：  
## 3.1 智能指针 https://blog.csdn.net/sjp11/article/details/123899141
...
## 5.3 移动语义  
将一个对象中资源移动到另外一个对象的方式。可以有效的缓解内存的压力，提高程序的运行效率，因为不需要开辟空间和释放空间  
## 6 模板  https://blog.csdn.net/JAN6055/article/details/123801017
模板定义并不是真正的定义了一个函数或者类，而是编译器根据程序员缩写的模板和形参来自己写出一个对应版本的定义，这个过程叫做模板实例化。编译器成成的版本通常被称为模板的实例。编译器为程序员生成对应版本的具体过程。类似宏替换。模板类在没有调用之前是不会生成代码的。由于编译器并不会直接编译模板本身，所以模板的定义通常放在头文件中。
